...
{
    private const ROUTE_PACKAGE_USAGE           = 'api.user.order.packages.usage';
    private const ROUTE_PACKAGE_GET_QR          = 'api.user.order.packages.getQR';
    private const ROUTE_PACKAGE_REEDEM_QR       = 'api.user.order.packages.reedemQR';
    ...
    protected function passedValidation(): void
    {
        ...
        if($route === self::ROUTE_PACKAGE_GET_QR || $route === self::ROUTE_PACKAGE_REEDEM_QR)
    ...
    public function validateOrderID($order_data)
    {
        ...
        if ($route === self::ROUTE_PACKAGE_REEDEM_QR)
        {
    ...
    private function requested($key)
    {
        $route = $this->route()->getName();
        $data = match ($route) {
            self::ROUTE_PACKAGE_USAGE                => $this->packageUsageRequest(),
            self::ROUTE_PACKAGE_GET_QR               => $this->packageGetQRRequest(),
            self::ROUTE_PACKAGE_REEDEM_QR            => $this->packageGetQRRequest(),
