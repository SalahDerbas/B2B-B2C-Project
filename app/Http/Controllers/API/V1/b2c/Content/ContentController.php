...
namespace App\Http\Controllers\API\V1\b2c\Content;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Http\Requests\API\V1\b2c\Content\ContentRequest;
...
class ContentController extends Controller
{
    ...
    {
        $data = getValueLookups($key);

        if (is_null($data))
            return responseSuccess('', getStatusText(CONTENT_EMPTY_CODE), CONTENT_EMPTY_CODE);

        return responseSuccess($data, getStatusText(CONTENT_SUCCESS_CODE), CONTENT_SUCCESS_CODE);
    }
