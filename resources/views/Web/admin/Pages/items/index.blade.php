    ...
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                <div class="row ">
                    <form action="{{ route('admin.items.index') }}" method="GET" class="d-flex align-items-end w-100">
                        <div class="col-3">
                        ...
                        <div class="col-2">
                            <a href="{{ route('admin.items.export', ['sub_category_id' => request('sub_category_id') ]) }}" class="btn btn-info text-center ">Export Excel</a>
                ...
                <br />
                <div class="row">
                    <a href="{{ route('admin.items.create') }}" class="btn btn-success text-center AddButtonC" >Add Item</a>
                </div>
                    ...
                    <tbody>
                        @forelse ($datas as $index => $item)
                            <tr>
                                ...
                                <td>
                                    @if($item->is_slider)
                                        <span class="btn btn-sm btn-outline-success round">True   </span>
                                    @else
                                        <span class="btn btn-sm btn-outline-danger round">False  </span>
                                    @endif
                                </td>
                                <td>{{ $item->getItemSource->package_id ?? 'N/A' }}</td>
                                <td>
                                    <span class="btn btn-sm btn-outline-primary round">{{ $item->getItemSource->getSource->name ?? 'N/A' }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdownMenuC" aria-labelledby="dropdownMenuButton" style="width: 184%;"  >
                                            ...
                                            <form action="{{ route('admin.items.edit') }}" method="GET" class="formC" >
                                                @csrf
                                                <input id="id" type="hidden" name="id" class="form-control" value="{{ $item->id }}">
                                                <button type="submit" class="dropdown-item d-flex justify-content-between align-items-center">
                                            ...
                                            <form action="{{ route('admin.items.switchStatus') }}" method="GET" class="formC" >
                                                @csrf
                                                <input id="id" type="hidden" name="id" class="form-control" value="{{ $item->id }}">
                                                <button type="submit" class="dropdown-item d-flex justify-content-between align-items-center">
                                                    @if($item->status)  Disable Status @else  Enable Status @endif
                                                    <i class="fa fa-ban" aria-hidden="true"></i>
                                                </button>
                                            </form>

                                            <form action="{{ route('admin.items.switchSlider') }}" method="GET" class="formC" >
                                                @csrf
                                                <input id="id" type="hidden" name="id" class="form-control" value="{{ $item->id }}">
                                                <button type="submit" class="dropdown-item d-flex justify-content-between align-items-center">
                                                    @if($item->is_slider)  Disable Slider @else  Enable Slider @endif
                                                    <i class="fa fa-ban" aria-hidden="true"></i>
                                                </button>
                                            </form>
